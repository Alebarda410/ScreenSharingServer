...
namespace Telepathy
{
    public class Server : Common
    {
        ...
        public bool Start(int port)
        {
            ...
            // ожидание необходимо для возвращения рального результата метода Start
            // по умолчанию старт listenerThread считается успешным при запуске
            ...
            if (message.Count <= MaxMessageSize)
            {
                ...
                if (clients.TryGetValue(connectionId, out ConnectionState connection))
                {
                    ...
                    else
                    {
                        ...
                        connection.sendPipe.Clear();
                        // just close it. send thread will take care of the rest.
                        //вернуть connection.client.Close();
